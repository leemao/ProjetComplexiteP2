...
/**
 * Calcul si il existe un stable de taille K ou plus.
 * @param G Graphe non-orienté
 * @param K taille minimum du stable calculé
 * @return 1 si il existe un stable de taille K ou plus, 0 sinon
 * @author Tristan NARI
 * @complexite O(2^n) dans le pire des cas ou il en trouve pas
 */
 int kStable(graphe_l G, int K){
    ens_de_sommets e;
	init_ens_de_sommets_bool(&e,G.n);
    return kStable_rec(G,K,e);
 }

/**
 * Récursion du calcul de l'existence d'un stable de taille K ou plus.
 * @param G Graphe non-orienté
 * @param K taille minimum du stable calculé
 * @param e ensemble de sommets
 * @return 1 si il existe un stable de taille K ou plus, 0 sinon
 ...
 * @complexite O(2^n) dans le pire des cas ou il en trouve pas
 */
 int kStable_rec(graphe_l G, int K, ens_de_sommets e){
    int i,j,card;
